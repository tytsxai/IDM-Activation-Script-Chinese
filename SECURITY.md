...
仅对 **`main` 分支上的当前版本** 提供安全修复。较早版本不再回溯。

当前版本号见 [`CHANGELOG.md`](./CHANGELOG.md) 顶部、README 页首的版本徽章，或运行脚本时的窗口标题（三者由 CI 强制一致）。本项目不使用 GitHub Releases，仓库内也没有 tag。
...
- 受影响的文件或版本（commit SHA，或 `CHANGELOG.md` 里的版本号）；
- 你对修复建议的初步想法（可选）。
...
- 发布产物（`release/` 下的 zip 与 `.sha256`）完整性校验问题。
