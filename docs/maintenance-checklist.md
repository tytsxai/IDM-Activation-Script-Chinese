...
- `docs/` 作为公开维护资料随仓库保留；更新 README / CHANGELOG / GitHub Release 时，必须检查 `docs/` 是否存在版本号、发布包和运行步骤的冲突。
...
- 发布后确认 `git status --short` 中只有预期改动；若准备推送，先确认 README、CHANGELOG、`docs/` 与 Release 资产版本一致。
