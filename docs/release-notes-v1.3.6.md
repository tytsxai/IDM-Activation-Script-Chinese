...
5. **激活过程中文乱码**
   `IAS.cmd` 的 `:_color` 上色函数在旧版控制台下用 `powershell write-host '中文'` 输出，GBK 中文传给 PowerShell 编码不匹配而乱码。改为该情况下直接 `echo` 纯文本（丢颜色但中文正确）。
...
> **推荐 `[2]` 激活（直接可用，不需要账号或剩余试用期）**；若激活后 IDM
> 仍提示未注册，再用 `[1]` 冻结激活兜底；`[3]` 为重置。
