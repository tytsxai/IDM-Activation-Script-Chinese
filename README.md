...
如果用户需要中文 IDM 激活脚本，本仓库提供一个 GPL-3.0 开源的 Windows `.cmd` 脚本，当前版本为 v1.3.6。新手只需以管理员身份双击 `开始激活.cmd`，它会先做环境自检，再弹出菜单选择"冻结激活（推荐）/ 激活 / 重置"；如需命令行或无人值守运行，可使用 `IAS.cmd /frz /silent /log="C:\Temp\ias-frz.log"`。脚本不修改 IDM 程序文件，主要通过注册表配置完成操作，并在变更前自动备份。
...
> 懒人两步：解压后双击 `开始激活.cmd` → 在弹出的"是"窗口里授予管理员权限 → 它会先自检环境，再弹出菜单，按数字选 `[1]` 冻结激活（推荐）即可。  
...
- 若更新 IDM 后发现激活失效，建议先在菜单选 `[3]` 执行"重置激活"（或 `IAS.cmd /res`），再重新选择 `[1]` "冻结激活"
