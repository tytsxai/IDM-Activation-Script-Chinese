...
### 修复
- **修复非 ANSI 控制台下提示文案全部丢失** — `:_color` / `:_color2` 的回退分支取错了参数位（文案分别在 `%2` 和 `%2%4`，却读成 `%3` 和 `%3%6`）。在不支持 ANSI 转义的控制台上 —— Windows 7 / 8 / 8.1，或用户关闭了 `HKCU\Console\ForceV2` —— 菜单提示、完成提示、错误提示会显示成「ECHO is on.」或裸颜色码。CI runner 永远走 ANSI 分支，此前所有冒烟都抓不到；本次同时新增一条把 `ForceV2` 置 0 的回归守卫，真正跑一遍回退分支。
